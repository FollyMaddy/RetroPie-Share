...
function mame_data_read() {
    ...
    if [[ -z ${mamedev_csv[@]} ]]; then
        if [[ -f /opt/retropie/emulators/mame/mame0249_systems_sorted_info ]]; then 
    clear
    echo "Get mame0249 data:/opt/retropie/emulators/mame/mame0249_systems_sorted_info"
    ...
    while read system_read;do mamedev_csv+=("$system_read");done < <(echo \",,,,\";cat /opt/retropie/emulators/mame/mame0249_systems_sorted_info|sed 's/,//g;s/Driver /\",/g;s/ ./,/;s/'\)':/,run_generator_script,/;s/\r/,,,\"/')
        else
    echo "Get mame0249 data:RetroPie-Share repository"
    echo "For speed, data will be re-used within this session"
    echo "Be patient for 20 seconds" 
    while read system_read;do mamedev_csv+=("$system_read");done < <(echo \",,,,\";curl https://raw.githubusercontent.com/FollyMaddy/RetroPie-Share/main/00-databases-00/mame/mame0249_systems_sorted_info|sed 's/,//g;s/Driver /\",/g;s/ ./,/;s/'\)':/,run_generator_script,/;s/\r/,,,\"/')
        fi
    fi
}
...
#remember : the first search option will be changed by the script to get search options beginning with, if you want a global search  do something like this : '//&&/hdv/'
    ...
    local rompack_link_info=( "mame-merged \Z2(0.249)" ".zip" "mame-merged/mame-merged/" )
    local csv=()
    csv=(
",menu_item,,to_do,"
...
",RetroPie/downloads < (NEW-SET)mame-sl  \Z2(0.249),,subform_add-mamedev-systems_downloads_wget_A '//' /home/$user/RetroPie/downloads/mame-sl mame-sl/mame-sl/ download,,,,,dialog_message \"NO HELP\","
",RetroPie/downloads < (NEW-SET)mame-merged  \Z2(0.249),,subform_add-mamedev-systems_downloads_wget_A '//' /home/$user/RetroPie/downloads/mame-merged mame-merged/mame-merged/ download,,,,,dialog_message \"NO HELP\","
