...
## `/opt/archypie/configs/SYSTEM/emulators.cfg` with \c \%ROM%, \c \%BASENAME%
## being replaced with the `ROM` parameter. This is the default mode used when
## launching in ArchyPie so the user can switch emulator used as well as other
...
function show_launch() {
    ...
    if [[ "$IS_SYS" -eq 1 && "$USE_ART" -eq 1 ]]; then
        ...
        images+=(
            ...
            "$HOME/ArchyPie/roms/$SYSTEM/media/screenshots/${ROM_BN}"
        )
    fi
    ...
    if [[ "$IS_SYS" -eq 1 ]]; then
        images+=(
            "$HOME/ArchyPie/roms/$SYSTEM/images/${ROM_BN}-launching"
            "$CONF_ROOT/launching"
        )
    fi
