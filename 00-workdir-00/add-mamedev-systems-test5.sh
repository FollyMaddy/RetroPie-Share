...
function gui_add() {
    ...
    #fixed array generated with : for i in ${!csv[@]}; do echo -n "\"${csv[$i]}\" " ;done >item
    local items_data=(
    "Handhelds / Plug & play -> Select and install,choose_dteam_add"
    "Handhelds / Plug & play -> Select downloads,gui_downloads"
    "-,-"
    "All -> Select and install upon descriptions,choose_description_add"
    "All -> Select and install upon system names,choose_system_add"
    "-,-"
    ) 
...
function build_menu() {
    while true; do
        ...
        if [[ -n "$choice" ]]; then
            ...
            clear
            "$(echo ${csv[$i]} | cut -d ',' -f 2)"
            #run_generator_script
            rp_registerAllModules
            echo $choice ${csv[$choice]}
            sleep 4
            joy2keyStop
            joy2keyStart
        else
