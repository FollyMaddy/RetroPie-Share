...
echo "generate and write the install-<RPname>-from-mame-system-<MESSname><-media>.sh script file(s)"
...
for index in "${!systems[@]}"; do sleep 0.001; [[ -n ${allextensions[$index]} ]] && cat > "scriptmodules/libretrocores/install-${newsystems[$index]}-from-mame-system-${systems[$index]}${media[$index]}".sh << _EOF_
#!/usr/bin/env bash
...
rp_module_id="install-${newsystems[$index]}-from-mame-system-${systems[$index]}${media[$index]}"
...
function depends_install-${newsystems[$index]}-from-mame-system-${systems[$index]}${media[$index]}() {
...
function sources_install-${newsystems[$index]}-from-mame-system-${systems[$index]}${media[$index]}() {
	true
}

function build_install-${newsystems[$index]}-from-mame-system-${systems[$index]}${media[$index]}() {
	true
}

function install_install-${newsystems[$index]}-from-mame-system-${systems[$index]}${media[$index]}() {
	true
}

function configure_install-${newsystems[$index]}-from-mame-system-${systems[$index]}${media[$index]}() {
	local _mess=\$(dirname "\$md_inst")/lr-mess/mess_libretro.so
	...
	addEmulator 0 "lr-mess-system-${systems[$index]}${media[$index]}" "\$_system" "\$_script \$_retroarch_bin \$_mess \$_config \\${systems[$index]} \$biosdir $messcfgoptions ${media[$index]} %ROM%"
...
function configure_install-${newsystems[$index]}-cmd() {
    ...
    addEmulator 0 "mame-basename" "\$_system" "/opt/retropie/emulators/mame/mame -v -c %BASENAME%"
    addEmulator 0 "mame-basename-autoframeskip" "\$_system" "/opt/retropie/emulators/mame/mame -v -c -autoframeskip %BASENAME%"
