...
rp_module_id="lr-mess-pockstat"
rp_module_name="Sony PocketStation"
rp_module_ext=".zip .gme"
...
function depends_lr-mess-pockstat() {
...
function sources_lr-mess-pockstat() {
	true
}

function build_lr-mess-pockstat() {
	true
}

function install_lr-mess-pockstat() {
	true
}

function configure_lr-mess-pockstat() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="pockstat"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config pockstat $biosdir -cart %ROM%"
